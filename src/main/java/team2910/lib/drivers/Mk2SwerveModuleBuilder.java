...
import team2910.lib.control.PidConstants;
import team2910.lib.control.PidController;
import team2910.lib.math.Vector2;
...
public class Mk2SwerveModuleBuilder {
    ...
    public Mk2SwerveModuleBuilder angleMotor(CANSparkMax motor, MotorType motorType) {
        ...
        return angleMotor((CANSparkMax) motor, motorType);
    }
    ...
    public Mk2SwerveModuleBuilder angleMotor(MotorController motor, MotorType motorType) {
        switch (motorType) {
            case CIM:
                ...
                // controller
                if (motor instanceof CANSparkMax || motor instanceof CANSparkMax) {
    ...
    public Mk2SwerveModuleBuilder driveMotor(MotorController motor, MotorType motorType) {
        // Spark MAXs are special and drive brushed motors in the opposite direction of every other motor controller
        if (motorType != MotorType.NEO && (motor instanceof CANSparkMax || motor instanceof CANSparkMax)) {
            motor.setInverted(true);
        }
